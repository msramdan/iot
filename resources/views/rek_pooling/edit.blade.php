...
@section('title', 'Edit Pooling Account')
                    ...
                    <div class="card-body">
                        <form action="{{ route('rek_pooling.update', $rek_pooling->id) }}" method="POST">
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-control @error('bank_id') @enderror" name="bank_id" id="bank_id">
                                    @foreach ($bank as $item)
                                      <option value="{{ $item->id }}" {{ (old('bank_id') ? old('bank_id') : $rek_pooling->bank_id) == $item->id ? 'selected' : ''}}>{{ $item->bank_name }}</option>
                                    @endforeach
                                </select>
                                @error('bank_id')
                                <span style="color: red;">{{ $message }}</span>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control @error('account_name') is-invalid @enderror" name="account_name" id="account_name" placeholder="" value="{{ old('account_name') ? old('account_name') : $rek_pooling->account_name }}" autocomplete="off">
                                @error('account_name')
                                <span style="color: red;">{{ $message }}</span>
                                @enderror
                            </div>
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control @error('number_account') is-invalid @enderror" name="number_account" id="number_account" placeholder="" value="{{ old('number_account') ? old('number_account') : $rek_pooling->number_account }}" autocomplete="off">
