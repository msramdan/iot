                    ...
                    <div class="card-body">
                        <form action="{{ route('village.store') }}" method="POST">
                            ...
                            <div class="mb-3">
                                ...
                                <select name="kecamatan_id" id="kecamatan_id" class="form-control">
                                    ...
                                    @foreach ($kecamatan as $v)
                                        <option value="{{ $v->id }}" {{ $v->id == old('kecamatan_id') ? 'selected' : '' }}>{{ $v->kecamatan }}</option>
                                    @endforeach
                                </select>
                                @error('kecamatan_id')
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control @error('kelurahan') is-invalid @enderror" name="kelurahan" id="kelurahan" placeholder="" value="{{ old('kelurahan') }}" autocomplete="off">
                                @error('kelurahan')
                                    <span style="color: red;">{{ $message }}</span>
                                @enderror
                            </div>
