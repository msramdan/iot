    ...
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="col-md-6">
                <div class="card">
                    <div class="card-body">
                        <form action="{{ route('otp.update', $otp->id) }}" method="POST">
                            @csrf
                            @method('PUT')
                            <div class="mb-3">
                                ...
                                <select name="merchant_id" id="merchant_id" class="form-control @error('merchant_id') is-invalid @enderror">
                                    ...
                                    @foreach ($merchants as $merchant)
                                        <option value="{{ $merchant->id }}" data-email="{{ $merchant->email }}" {{ $otp->merchant_id == $merchant->id ? 'selected' : '' }}>{{ $merchant->merchant_name }}</option>
                                    @endforeach
                                </select>
                                @error('merchant_id')
                            ...
                            <div class="mb-3">
                                ...
                                <input type="email" class="form-control @error('email') is-invalid @enderror"
                                    ...
                                    value="{{ old('email') ?? $otp->email }}" autocomplete="off">
                                @error('email')
                                <span style="color: red;">{{ $message }}</span>
                                @enderror
                            </div>
                            <div class="mb-3">
                                ...
                                <input type="number" class="form-control @error('otp_number') is-invalid @enderror"
                                    ...
                                    value="{{ old('otp_number') ?? $otp->otp_number }}" autocomplete="off">
                                @error('otp_number')
                                <span style="color: red;">{{ $message }}</span>
                                @enderror
                            </div>
                            <div class="mb-3">
                                ...
                                <input type="date" name="expired_date" value="{{ old('expired_date') ?? $otp->expired_date }}" id="expired_date" class="form-control @error('expired_date') is-invalid @enderror" >
                                @error('expired_date')
                                <span style="color: red;">{{ $message }}</span>
                                @enderror
                            </div>
